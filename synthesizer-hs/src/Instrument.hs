...
staticSaw :: (Trans.C a, RealField.C a, SigG.Write sig a) =>
   SigG.LazySize ->
   Phase.T a -> a -> sig a
staticSaw lazySize = Osci.static defaultLazySize Wave.saw
...
boing frequency = let
  ...
  ampModulatingSine = staticSaw defaultLazySize zero (2.1233 / sampleRateF) :: SoundStream
